...
export default (info: ComponentInfo) => {
	...
	const afterLines = fileContentLine.slice(
		newTypeIndex,
		fileContent.length - 1
	);

	// check if type is already exist
	const componentTypeList = beforeLines
		.map((line: string) =>
			line.replace(/\s+/g, '').replace(/,/g, '').replace(/"/g, '')
		)
		.slice(1);
	const isComponentExist = componentTypeList.some(
		(type) => type === newTypeName
	);
